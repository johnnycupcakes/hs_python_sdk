...
#prints everything in creds file
print(creds)
...
while (menu_item is not 111):
    ...
    if menu_item is 1:
        print("****************************************************1 - client.get_signature_request_file STARTCOOL**********")
        client = HSClient(api_key=apikey)
        response = client.get_signature_request_file(
        signature_request_id='0b06ee180907c6185fa1fee02d6ef1287884a5a7',
        filename='files.pdf',
        file_type='pdf'
)
        print("****************************************************1 - client.get_signature_request_file ENDCOOL**********")
    elif menu_item is 2:
        ...
        response = client.send_signature_request(
                test_mode=True,
                title="Burgerboss NDA",
                subject="The NDA we talked about",
                message="Bob's Burgers",
                signers=[
                    { 'email_address': 'nicholas.boutte@hellosign.com', 'name': 'Bob Belcher', 'order': 0 }
                ],
                cc_email_addresses=[],
                files=['NDA.pdf']
            )
        ...
        response = client.send_signature_request_with_template(
                ...
                subject='Spongebob',
                message='Squarepants',
                signers=[{ 'role_name': '1', 'name': 'Tina Belcher', 'email_address': 'nicholas.boutte@hellosign.com' }]
            )
        print(response)
        print(type(client))
        ...
        print("****************************************************5 Embedded requesting non embedded signing - START**********************")
        client = HSClient(api_key=apikey)
        response = client.create_unclaimed_draft(
            test_mode=True,
        ...
        print(response)
        print(type(client))
        print("****************************************************5 Embedded requesting non embedded signing - END**********************")
    elif menu_item is 6:
        print("****************************6 - Embedded Signature request with template - START**********************")
        client = HSClient(api_key=apikey)
        response = client.send_signature_request_embedded_with_template(
            test_mode=True,
            client_id='44adc0a3f5f6b105ca6386e164d1e87a',
            template_id='85eb6d1cdbca7780f9e875697c36bc762e0fec67',
            title='Bobs Burgers',
            subject='The NDA we talked about',
            message='Please sign this NDA and then we can discuss more. Let me know if you have any questions.',
            signers=[
                { 'role_name': '1', 'email_address': 'nicholas.boutte+1@hellosign.com', 'name': 'Burgerboss', 'order': 0 }
            ]
        )
        print(response)
        print(type(client))
        ...
        print("****************************7 - Embedded Signature request with document - START**********************")
        client = HSClient(api_key=apikey)
        response = client.send_signature_request_embedded(
            ...
            message='Please sign this NDA and then we can discuss more. Let me know if you have any questions.',
            signers=[
                { 'email_address': 'nicholas.boutte+1@hellosign.com', 'name': 'Bob Belcher', 'order': 0 }
            ]
        )
        print(response)
        print(type(client))
        print("****************************7 - Embedded Signature request with document - END**********************")
    elif menu_item is 8:
        ...
        print("**********************12 Create embedded unclaimed draft with template - START**********************")
        client = HSClient(api_key=apikey)
        ...
        templateDraft = client.create_embedded_unclaimed_draft_with_template(
            ...
            template_id='85eb6d1cdbca7780f9e875697c36bc762e0fec67',
            requester_email_address='nicholas.boutte@hellosign.com', 
            title='MyDraft', 
